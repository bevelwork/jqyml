...
make test-doom-jq-rig        # game rig: run input sequences, assert state (cases/*.json)
...
| **test-doom-jq-rig** | Runs `doom_jq/tests/game_rig.py` on each `doom_jq/tests/cases/*.json`: starts from a preset (e.g. `game_center`, `game_south_wall`), sends a sequence of key inputs, asserts expected `player` state after each step. Use for movement/backward/slide regression. |
